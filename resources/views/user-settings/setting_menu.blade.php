<div id="mySidebar" class="me-5 sidebar d-lg-block d-xl-block">
    ...
    <ul class="d-flex nav nav-tabs mb-5 pb-1 overflow-auto flex-nowrap text-nowrap flex-column setting-tab" id="myTab" role="tablist">
        ...
        <li class=" nav-item position-relative" role="presentation">
            <a class="nav-link me-0 p-0 {{ (isset($sectionName) && $sectionName == 'open_ai') ? 'active' : '' }}"
               href="{{ route('user.setting.index',['section' => 'open_ai']) }}" data-turbo="false"><i class="fa-solid fa-gear"></i> &nbsp;{{__('messages.vcard.open_ai') }}</a>
        </li>
        {{-- @if ($isAllowCustomDomain)
            <li class=" nav-item position-relative" role="presentation">
                <a class="nav-link me-0 p-0 {{ isset($sectionName) && $sectionName == 'custom_domain' ? 'active' : '' }}"
                    href="{{ route('user.setting.index', ['section' => 'custom_domain']) }}" data-turbo="false"><i
                        class="fa-solid fa-globe icon-color-bs-blue"></i> &nbsp;{{ __('messages.custom_domain.custom_domain') }}</a>
            </li>
        @endif --}}
    </ul>
</div>
