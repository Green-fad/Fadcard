    ...
    <div class="container p-0 vcard22-main">
        <div class="vcard22-main main-content w-100 mx-auto content-blur allSection collapse show">
            <div class="vcard-one__product py-3 mt-0 px-30">
                <div class="d-flex align-items-center gap-2 mb-4 justify-content-between" @if (getLanguage($vcard->default_language) == 'Arabic') dir="rtl" @endif>
                    <div>
                        <h4 class="vcard-prodcut-heading mb-0">{{ __('messages.vcard.products') }}</h4>
                    </div>
                    <div class="text-center">
                        <a class="back-btn text-decoration-none d-block" id="product-back"
                            href="{{ $vcardUrl }}"
                ...
                <div class="product-slider">
                    @foreach ($vcard->products as $product)
                        <div class="">
                            <div class="card product-card product-back mb-3">
                                ...
                                <div class="card-body card-padding px-0 pb-0">
                                    <div class="product-desc">
                                        <h3 class="product-head fs-20 fw-5 mb-2">{{ $product->name }}</h3>
                                    </div>
                                    <p class="fs-14 text-gray-100 mb-2">{{ $product->description }}</p>
                                    <div class="product-amount text-primary fw-bold fs-20 mb-3">
                                            @if ($product->currency_id && $product->price)
                                                <span
                                                    class="fs-20 fw-6 product-price-{{ $product->id }}">{{ $product->currency->currency_icon }}{{ getSuperAdminSettingValue('hide_decimal_values') == 1 ? number_format($product->price, 0) : number_format($product->price, 2) }}</span>
                                            @elseif($product->price)
                                                <span
                                                    class="fs-20 fw-6  product-price-{{ $product->id }}">{{ getUserCurrencyIcon($vcard->user->id) }}{{ $product->price }}</span>
                                            @endif
                                        </div>
